document.addEventListener('DOMContentLoaded', () => {
    ...
    const DEFAULT_PASSWORD = window.APP_DEFAULTS ? window.APP_DEFAULTS.DEFAULT_PASSWORD : null;
    ...
    function updateFontLabel(index) {
	 ...
	 // ========== [START] ฟังก์ชันช่วยสำหรับการค้นหาด้วยเสียง ==========
    const thaiMonthMap = {
        'มกรา': 1, 'มกราคม': 1, 'ม.ค.': 1,
        'กุมภา': 2, 'กุมภาพันธ์': 2, 'ก.พ.': 2,
        'มีนา': 3, 'มีนาคม': 3, 'มี.ค.': 3,
        'เมษา': 4, 'เมษายน': 4, 'เม.ย.': 4,
        'พฤษภา': 5, 'พฤษภาคม': 5, 'พ.ค.': 5,
        'มิถุนา': 6, 'มิถุนายน': 6, 'มิ.ย.': 6,
        'กรกฎา': 7, 'กรกฎาคม': 7, 'ก.ค.': 7,
        'สิงหา': 8, 'สิงหาคม': 8, 'ส.ค.': 8,
        'กันยา': 9, 'กันยายน': 9, 'ก.ย.': 9,
        'ตุลา': 10, 'ตุลาคม': 10, 'ต.ค.': 10,
        'พฤศจิกา': 11, 'พฤศจิกายน': 11, 'พ.ย.': 11,
        'ธันวา': 12, 'ธันวาคม': 12, 'ธ.ค.': 12
    };

    function parseYearFromText(text) {
        if (text.includes('ปีที่แล้ว')) return new Date().getFullYear() - 1;
        if (text.includes('ปีนี้')) return new Date().getFullYear();
        if (text.includes('ปีหน้า')) return new Date().getFullYear() + 1;
    ...
    function trimSearchPrefix(str) {
        const searchPrefixes = ['ค้นหา', 'หา', 'ดู', 'filter', 'กรอง', 'search'];
    ...
    async function saveToCloud(storeName, item) {
        if (window.auth && window.auth.currentUser && window.db) {
            try {
				...
				const now = new Date();                     // ✅ ประกาศตัวแปร
				const isoNow = now.toISOString();
				state.cloudSyncLastTime = isoNow;
				await updateCloudSyncTimeInDB(isoNow);      // ฟังก์ชันที่เราสร้าง

				// ถ้าผู้ใช้กำลังอยู่ในหน้า Settings ให้อัปเดตข้อความทันที
				if (currentPage === 'page-settings') {
					const cloudSpan = document.getElementById('last-cloud-sync-time');
					if (cloudSpan) {
						cloudSpan.textContent = new Date(now).toLocaleString('th-TH');
					}
				}
    ...
    async function deleteFromCloud(storeName, key) {
        if (window.auth && window.auth.currentUser && window.db) {
            try {
				...
				// อัปเดตเวลา Cloud Sync
				const now = new Date().toISOString();
				state.cloudSyncLastTime = now;
				await updateCloudSyncTimeInDB(now);
    ...
    window.clearLocalDataForLogout = async () => {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
				...
				configStore.add({ key: 'password', value: null });  // ไม่มีรหัสผ่านเริ่มต้น
	...
	function updateCloudSyncTimeInDB(timestamp) {
		return new Promise((resolve, reject) => {
			if (!db) return reject("DB not initialized");
			const tx = db.transaction([STORE_CONFIG], 'readwrite');
			const store = tx.objectStore(STORE_CONFIG);
			const request = store.put({ key: 'cloudSyncLastTime', value: timestamp });
			request.onsuccess = () => resolve();
    ...
    let state = {
		...
		manualBackupLastTime: null,   // เวลาสำรองแบบ Manual (JSON)
		cloudSyncLastTime: null,      // เวลาซิงค์ขึ้น Cloud แบบ Manual
		showVoiceTooltip: true, // ค่าเริ่มต้น: แสดงคำแนะนำ
        settingsCollapse: {},
        autoLockTimeout: 0,
		budgets: [],
		notifySettings: {
    ...
    async function loadStateFromDB() {
        try {
			...
			const passwordTypeConfig = await dbGet(STORE_CONFIG, 'passwordType');
			state.passwordType = passwordTypeConfig ? passwordTypeConfig.value : 'text';
			// +++++++++++++++++++++++++++++++++++++++++++++++++

            if (passwordConfig) {
				storedPassword = passwordConfig.value;
			} else {
				// ไม่มีรหัสผ่านใน DB → ไม่สร้างรหัสผ่านเริ่มต้น
				storedPassword = null;
				// ถ้ายังไม่ได้เซ็ต state.password ที่อื่น ให้เซ็ตตรงนี้
				// state.password = null;   // (รอเซ็ตจาก storedPassword ท้ายสุด)
			}
			...
			if (autoBackupConfig) {
				state.autoBackup = autoBackupConfig.value;
			} else {
				state.autoBackup = { enabled: false, frequency: 'daily', lastBackup: null };
			}
			
			// +++ เพิ่มใหม่: โหลดเวลาสำรองเองและซิงค์คลาวด์ +++
			const manualBackupTime = await dbGet(STORE_CONFIG, 'manualBackupLastTime');
			state.manualBackupLastTime = manualBackupTime ? manualBackupTime.value : null;

			const cloudSyncTime = await dbGet(STORE_CONFIG, 'cloudSyncLastTime');
			state.cloudSyncLastTime = cloudSyncTime ? cloudSyncTime.value : null;
			// ++++++++++++++++++++++++++++++++++++++++++++++
		...
		// โหลดค่าการแสดงคำแนะนำปุ่มไมค์
		const voiceTooltipConfig = await dbGet(STORE_CONFIG, 'showVoiceTooltip');
		if (voiceTooltipConfig) {
			state.showVoiceTooltip = voiceTooltipConfig.value;
		}
		...
		async function checkAndProcessRecurring() {
			try {
				...
				// --- ใช้วันที่ตามเวลาท้องถิ่น (Local Time) แทน UTC ---
				const now = new Date();
				const year = now.getFullYear();
				...
				for (const rule of rules) {
					...
					while (nextDate <= today) {
						...
						const newTx = {
							...
							// [แก้บัค] เปลี่ยนจาก 08:00 เป็น 00:00 เพื่อให้ถือว่าถึงเวลาจ่ายทันทีที่ขึ้นวันใหม่ ไม่ต้องรอ 8 โมง
							date: `${nextDate}T00:00`, 
				...
				if (processedCount > 0) {
					...
					if (currentPage === 'calendar') renderCalendarView(); // สำคัญ: เพื่อให้สีเหลืองหายไปและสีแดง/เขียวโผล่มาแทน
				}
			...
			// ============================================
			// [ส่วนที่เพิ่มใหม่] ฟังก์ชันซ่อมแซมรายการเก่าที่ค้างสถานะ "รอจ่าย"
			// ============================================
			try {
				const allExistingTx = await dbGetAll(STORE_TRANSACTIONS);
				let oldTxFixedCount = 0;

				for (let tx of allExistingTx) {
					// ค้นหารายการเก่าที่มีเวลา T08:00 ติดอยู่ (ซึ่งเป็นโค้ดเก่าที่ทำให้ค้างรอจ่าย)
					if (tx.date && tx.date.includes('T08:00')) {
						// แก้ไขเวลาในฐานข้อมูลให้เป็น T00:00 (เพื่อให้ระบบมองว่าถึงเวลาจ่ายแล้ว)
						tx.date = tx.date.replace('T08:00', 'T00:00');
						await dbPut(STORE_TRANSACTIONS, tx);
						oldTxFixedCount++;
					}
				}

				if (oldTxFixedCount > 0) {
					// โหลดข้อมูลที่แก้แล้วเข้าระบบใหม่
					state.transactions = await dbGetAll(STORE_TRANSACTIONS);
					
					// รีเฟรชหน้าจอเพื่อให้สีเปลี่ยนจากเหลือง (รอจ่าย) เป็นสีแดง (จ่ายแล้ว)
					if (typeof renderAll === 'function') renderAll();
					if (typeof renderListPage === 'function') renderListPage();
					if (typeof renderCalendarView === 'function') renderCalendarView();
					
					showToast(`อัปเดตสถานะรายการเก่าสำเร็จ ${oldTxFixedCount} รายการ`, "success");
				}
			} catch (fixErr) {
				console.error("Error fixing old pending transactions:", fixErr);
			}
			// ============================================
		}
		...
		async function initApp() {
			try {
				...
				if (state.password) {
					...
					if (unlockInput) {
						if (state.passwordType === 'pin') {
							unlockInput.setAttribute('inputmode', 'numeric');
							unlockInput.setAttribute('pattern', '\\d*');
						} else {
							unlockInput.removeAttribute('inputmode');
							unlockInput.removeAttribute('pattern');
						}
					}
					
					// +++ เพิ่มส่วนนี้: ปรับ inputmode ของช่องรหัสผ่านตามประเภท +++
					const unlockPasswordEl = document.getElementById('unlock-password');
					if (unlockPasswordEl) {
						if (state.passwordType === 'pin') {
							unlockPasswordEl.setAttribute('inputmode', 'numeric');
							unlockPasswordEl.setAttribute('pattern', '\\d*');
						} else {
							unlockPasswordEl.removeAttribute('inputmode');
							unlockPasswordEl.removeAttribute('pattern');
						}
					}
		...
		// ============================================
		// รีเฟรชข้อมูลจากคลาวด์ (เหมือนเปิดแอปใหม่)
		// ============================================
		async function refreshFromCloud() {
			if (!window.auth || !window.auth.currentUser) {
				showToast("กรุณาเข้าสู่ระบบก่อนรีเฟรชข้อมูล", "warning");
				return;
			}

			const refreshDesktop = document.getElementById('refresh-cloud-desktop');
			const refreshMobile = document.getElementById('refresh-cloud-mobile');
			const icons = [];

			if (refreshDesktop) {
				const icon = refreshDesktop.querySelector('i');
				if (icon) icons.push(icon);
			}
			if (refreshMobile) {
				const icon = refreshMobile.querySelector('i');
				if (icon) icons.push(icon);
			}

			// เริ่มโหลด: ไอคอนหมุน + สีเขียว
			icons.forEach(icon => {
				icon.classList.add('fa-spin', 'text-green-500');
			...
			try {
				...
				showToast('รีเฟรชไม่สำเร็จ: ' + err.message, 'error');
			}
		}
		...
		function handleSummaryCardClick(type) {
			...
			// ฟังก์ชันช่วยแปลงวันที่ให้เป็น YYYY-MM-DD แบบไม่ติด Timezone (Local Time)
			...
			if (state.homeViewMode === 'month') {
				...
				state.advFilterStart = formatLocalDate(firstDay);
				state.advFilterEnd = formatLocalDate(lastDay);
			} else if (state.homeViewMode === 'year') {
				// ถ้าหน้าแรกดูรายปี -> ตั้งค่าเป็น 1 ม.ค. - 31 ธ.ค.
				state.advFilterStart = `${y}-01-01`;
				state.advFilterEnd = `${y}-12-31`;
			} else {
			...
			if (typeof updateSharedControls === 'function') updateSharedControls('list'); 
			showPage('page-list');
	...
	function setupEventListeners() {
        ...
        if (toggleSch) {
            ...
            toggleSch.addEventListener('change', async (e) => {
                ...
                showToast(`ตั้งค่าแจ้งเตือนล่วงหน้า ${e.target.checked ? 'เปิด' : 'ปิด'} เรียบร้อย`, 'success');
            });
        }
        ...
        if (toggleRec) {
            ...
            toggleRec.addEventListener('change', async (e) => {
                ...
                await dbPut(STORE_CONFIG, { key: 'notification_settings', value: state.notifySettings });
                showToast(`ตั้งค่าแจ้งเตือนรายการประจำ ${e.target.checked ? 'เปิด' : 'ปิด'} เรียบร้อย`, 'success');
            });
        }
		// ==========================================
		...
		if (refreshDesktop) {
			refreshDesktop.addEventListener('click', refreshFromCloud);
		}
		if (refreshMobile) {
			refreshMobile.addEventListener('click', refreshFromCloud);
		}
		
		// ดักจับ F5, Ctrl+R, Cmd+R เพื่อเรียก refreshFromCloud แทนการรีเฟรชหน้า
		document.addEventListener('keydown', (e) => {
			// ตรวจสอบว่าเป็น F5 หรือ (Ctrl+R) หรือ (Cmd+R)
			if (e.key === 'F5' || (e.ctrlKey && e.key === 'r') || (e.metaKey && e.key === 'r')) {
				e.preventDefault(); // ป้องกันการรีเฟรชหน้าจอปกติ
				refreshFromCloud(); // เรียกฟังก์ชันซิงค์ข้อมูล
			}
		});
        ...
        // [อัปเดตใหม่] ดักจับการเข้าเมนู Settings ทุกรูปแบบ (รวมถึงเมนู Bottom Nav ในมือถือ)
        const renderNotifyData = () => {
            setTimeout(() => {
                if(typeof renderCustomNotifyList === 'function') {
                    renderCustomNotifyList();
                }
            }, 100); // หน่วงเวลาเล็กน้อยให้เปลี่ยนหน้าเสร็จก่อน
        };

        // 1. ดักจับปุ่มเมนูบน Desktop
        const navSettingsBtnForNotify = document.getElementById('nav-settings');
        if (navSettingsBtnForNotify) navSettingsBtnForNotify.addEventListener('click', renderNotifyData);

        // 2. ดักจับปุ่มเมนู Hamburger (มุมบนซ้ายมือถือ)
        const navSettingsMobileBtnForNotify = document.getElementById('nav-settings-mobile');
        if (navSettingsMobileBtnForNotify) navSettingsMobileBtnForNotify.addEventListener('click', renderNotifyData);

        // 3. [สำคัญ] ดักจับปุ่มเมนูด้านล่างสุดของมือถือ (Bottom Nav)
        ...
        // 1. จัดการแสดงช่องเวลาเมื่อเลือกแบบระบุเวลาใน Modal
        document.querySelectorAll('input[name="modal-notify-time-type"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                const wrapper = document.getElementById('modal-notify-time-input-wrapper');
                if (wrapper) {
                    if (e.target.value === 'specific') {
        ...
        window.openCustomNotifyModal = function(dateStr = null, editIdx = null) {
            const modal = document.getElementById('custom-notify-modal');
            if (!modal) return;
            
            // รีเซ็ตฟอร์ม
            document.getElementById('custom-notify-idx').value = '';
            document.getElementById('modal-custom-notify-msg').value = '';
            document.getElementById('modal-custom-notify-days').value = '0';
            document.getElementById('modal-custom-notify-time').value = '';
            document.getElementById('modal-custom-notify-repeat').value = 'none';
            
            // ตั้งค่าวันที่ (ใช้ Local Date ไม่เพี้ยนตาม Timezone)
            if (dateStr) {
                document.getElementById('modal-custom-notify-date').value = dateStr;
            } else {
                const now = new Date();
                const y = now.getFullYear();
                const m = String(now.getMonth() + 1).padStart(2, '0');
                const d = String(now.getDate()).padStart(2, '0');
                document.getElementById('modal-custom-notify-date').value = `${y}-${m}-${d}`;
            }
            
            // คืนค่า Radio
            const radioAllDay = document.querySelector('input[name="modal-notify-time-type"][value="all1day"]');
            if (radioAllDay) {
                radioAllDay.checked = true;
                radioAllDay.dispatchEvent(new Event('change'));
            }
            
            document.getElementById('custom-notify-modal-title').innerText = 'เพิ่มการแจ้งเตือนใหม่';

            // กรณีแก้ไขข้อมูล
            if (editIdx !== null && editIdx !== undefined) {
                const item = state.customNotifications[editIdx];
                if (item) {
                    document.getElementById('custom-notify-idx').value = editIdx;
                    document.getElementById('modal-custom-notify-msg').value = item.message;
                    document.getElementById('modal-custom-notify-date').value = item.date;
                    document.getElementById('modal-custom-notify-days').value = item.advanceDays || 0;
                    document.getElementById('modal-custom-notify-repeat').value = item.repeat || 'none';
                    
                    if (!item.isAllDay) {
                        const radioSpecific = document.querySelector('input[name="modal-notify-time-type"][value="specific"]');
                        if (radioSpecific) {
                            radioSpecific.checked = true;
                            radioSpecific.dispatchEvent(new Event('change'));
                        }
                        document.getElementById('modal-custom-notify-time').value = item.time;
                    }
                    document.getElementById('custom-notify-modal-title').innerText = 'แก้ไขการแจ้งเตือน';
                }
            }
            
            modal.classList.remove('hidden');
            setTimeout(() => document.getElementById('modal-custom-notify-msg').focus(), 300);
        };

        // 3. ฟังก์ชันปิด Modal
        window.closeCustomNotifyModal = function() {
            const modal = document.getElementById('custom-notify-modal');
            if (modal) modal.classList.add('hidden');
        };
        
        window.handleEditCustomNotify = function(idx) {
            openCustomNotifyModal(null, idx);
        };
		
		// ==========================================
        // ฟังก์ชันเสริมสำหรับ ลบ/แก้ไข การแจ้งเตือนจากหน้าปฏิทิน
        // ==========================================
        window.editNotifyFromCal = function(dateStr, idx) {
            Swal.close(); // ปิดหน้าต่างปฏิทิน
            setTimeout(() => {
                if (typeof openCustomNotifyModal === 'function') {
                    openCustomNotifyModal(dateStr, idx);
                }
            }, 300);
        };

        window.deleteNotifyFromCal = async function(idx) {
            Swal.close(); // ปิดหน้าต่างปฏิทินก่อน เพื่อให้ลบได้อย่างราบรื่น
            setTimeout(async () => {
                ...
                if (confirm.isConfirmed) {
                    ...
                    if (typeof addActivityLog === 'function') {
                        addActivityLog('🗑️ ลบแจ้งเตือน', n.message, 'fa-bell', 'text-red-600');
                    }
                    
                    if (typeof renderCustomNotifyList === 'function') renderCustomNotifyList();
                    if (typeof renderCalendarView === 'function') renderCalendarView();
                    if (typeof showToast === 'function') showToast('ลบเรียบร้อย', 'success');
                }
            }, 300);
        };

        // 4. ผูก Event ปุ่มบันทึกใน Modal
        const modalBtnSaveCustom = document.getElementById('modal-btn-save-custom-notify');
        if (modalBtnSaveCustom) {
            modalBtnSaveCustom.addEventListener('click', async () => {
                const editIdx = document.getElementById('custom-notify-idx').value;
                const msg = document.getElementById('modal-custom-notify-msg').value.trim();
                const date = document.getElementById('modal-custom-notify-date').value;
                const days = document.getElementById('modal-custom-notify-days').value;
                const repeat = document.getElementById('modal-custom-notify-repeat').value;
                const timeTypeEl = document.querySelector('input[name="modal-notify-time-type"]:checked');
                const timeType = timeTypeEl ? timeTypeEl.value : 'all1day';
                let specificTime = null;

                if (!msg || !date) {
                    Swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอกข้อความและวันที่แจ้งเตือน', 'warning');
                    return;
                }
                if (timeType === 'specific') {
                    specificTime = document.getElementById('modal-custom-notify-time').value;
                    if (!specificTime) {
                        Swal.fire('ข้อมูลไม่ครบ', 'กรุณาระบุเวลาที่ต้องการให้เตือน', 'warning'); return;
                    }
                }
                ...
                addActivityLog(
                    editIdx !== '' ? '✏️ แก้ไขแจ้งเตือน' : '🔔 เพิ่มแจ้งเตือน',
                    `${msg} | 📅 วันที่: ${formattedNotifyDate}`,
                    'fa-bell',
                    'text-purple-600'
                );
        ...
        // 5. ฟังก์ชันวาดรายการ (Render) ให้สัมพันธ์กับ HTML ใหม่
        function renderCustomNotifyList() {
			...
			if (!state.customNotifications || state.customNotifications.length === 0) {
                list.innerHTML = '<p class="text-gray-400 dark:text-gray-500 text-center py-4 border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-xl text-sm">ยังไม่มีรายการแจ้งเตือน</p>';
				return;
			}
			...
			state.customNotifications.forEach((n, idx) => {
				...
				if (n.isAllDay === false && n.time) { 
					timeDisplay = `<span class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-[10px] font-bold whitespace-nowrap"><i class="fa-regular fa-clock mr-1"></i>${n.time} น.</span>`;
				} else {
					timeDisplay = `<span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-[10px] whitespace-nowrap"><i class="fa-solid fa-sun mr-1"></i>ทั้งวัน</span>`;
				}
				
				let repeatBadge = '';
				if (n.repeat && n.repeat !== 'none') {
					const repeatLabels = { 'weekly': 'สัปดาห์', 'monthly': 'เดือน', 'yearly': 'ปี' };
					repeatBadge = `<span class="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[10px] font-bold border border-blue-200 whitespace-nowrap"><i class="fa-solid fa-rotate mr-1"></i>ทุก${repeatLabels[n.repeat]}</span>`;
				}

				const html = `
					<div class="flex justify-between items-start sm:items-center bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm mb-2 transition-colors gap-2">
						<div class="${statusClass} flex-grow min-w-0"> <div class="font-bold text-sm flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2">
								<span class="truncate block">${n.message}</span>
								<div class="flex flex-wrap gap-1 items-center">
									${timeDisplay} ${repeatBadge}
								</div>
							</div>
							<div class="text-xs mt-1.5 text-gray-500 dark:text-gray-400 flex items-center">
								<i class="fa-regular fa-calendar mr-1"></i>${dateStr} ${n.advanceDays > 0 ? `(เตือนก่อน ${n.advanceDays} วัน)` : ''}
							</div>
						</div>
			...
			document.querySelectorAll('.delete-custom-notify').forEach(btn => {
				btn.addEventListener('click', async (e) => {
					...
					const confirm = await Swal.fire({
						title: 'ลบรายการ?', text: 'ต้องการลบการแจ้งเตือนนี้ใช่ไหม', icon: 'warning',
		...
		if (advStart && advEnd) {
			...
			// ประกาศตัวแปรเก็บ timer (วางไว้ก่อน addEventListener)
			let searchTimeout;

			advSearch.addEventListener('input', () => {
				// ยกเลิก timer เก่าทุกครั้งที่มีการพิมพ์ใหม่
				clearTimeout(searchTimeout);
				
				// ตั้ง timer ใหม่ รอ 300ms แล้วค่อยเรียก renderListPage
		...
		const deleteAllBtn = document.getElementById('btn-delete-all-imported');
			...
			// ปุ่ม "เดือนนี้" ในหน้ารายการ
			const btnThisMonth = document.getElementById('btn-this-month');
			if (btnThisMonth) {
				btnThisMonth.addEventListener('click', resetToCurrentMonth);
			}
   ...
   function renderPieChart(transactions) {
			...
			try {
				...
				if (showMoney) {
					...
					moneyCalendar = new FullCalendar.Calendar(moneyContainer, {
						...
						eventClick: function(info) {
							showDailyDetails(info.event.startStr.slice(0, 10));
						},
						// [คงไว้] ปฏิทินเงิน โชว์แค่จุดสีส้มตามที่ต้องการ
						dayCellDidMount: function(arg) {
							...
							const hasNotify = state.customNotifications && state.customNotifications.some(n => {
							...
							if (hasNotify) {
								...
								dotWrapper.className = 'absolute top-1 left-1 z-10 flex items-center justify-center';
								dotWrapper.innerHTML = '<span class="w-2.5 h-2.5 bg-orange-500 rounded-full shadow-sm animate-pulse" title="มีการแจ้งเตือนพิเศษ"></span>';
								
								const frame = arg.el.querySelector('.fc-daygrid-day-frame');
								if (frame) {
									frame.style.position = 'relative';
									frame.appendChild(dotWrapper);
								}
							}
						},
				...
				if (showImported) {
					...
					importedCalendar = new FullCalendar.Calendar(importedContainer, {
						...
						showNonCurrentDates: false,
						fixedWeekCount: false,
						// [ใหม่] สร้าง event ให้แสดงข้อความแจ้งเตือนพิเศษ (แทนจุดสีส้ม)
						events: function(info, successCallback, failureCallback) {
							const eventsArr = [];
							
							// 1. ดึงกิจกรรมที่นำเข้า (ICS)
							...
							// 2. ดึงการแจ้งเตือนพิเศษ และสร้างเป็นข้อความแถบสีส้มในปฏิทินเลย
							...
							for (let d = new Date(start); d < end; d.setDate(d.getDate() + 1)) {
								...
								(state.customNotifications || []).forEach(n => {
									let isDue = false;
									if (typeof isNotificationDue === 'function') {
										isDue = isNotificationDue(n, dateStr);
									} else {
										isDue = (n.date === dateStr);
									}
									
									if (isDue) {
										eventsArr.push({
											id: 'notif_' + n.id + '_' + dateStr,
											title: '🔔 ' + n.message,
											start: dateStr,
											allDay: true,
											color: '#f97316', // สีส้มเพื่อแยกแยะให้ชัดเจน
											textColor: '#ffffff',
											classNames: ['custom-notify-event']
										});
									}
								});
							}
						...
						eventOrder: "start,-duration,allDay,title",
						dateClick: function(info) {
							showImportedDailyDetails(info.dateStr);
						},
						eventClick: function(info) {
							showImportedDailyDetails(info.event.startStr.slice(0, 10));
						},
						datesSet: function(info) {
							if (isSyncingCalendars) return;
							isSyncingCalendars = true;
	...
	async function showImportedDailyDetails(dateStr) {
			...
			if (dailyImported.length === 0 && dailyNotif.length === 0) {
				let emptyHtml = `
					<div class="py-6 text-center">
						<i class="fa-regular fa-calendar-xmark text-4xl text-gray-300 dark:text-gray-600 mb-3 block"></i>
						<p class="text-gray-500 dark:text-gray-400">วันนี้ไม่มีกิจกรรมที่นำเข้าและการแจ้งเตือนพิเศษ</p>
					</div>
					<div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
						<button id="cal-imported-add-notify-btn-empty" class="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-2.5 px-4 rounded-xl shadow-md transition duration-300 flex items-center justify-center gap-2 text-sm">
							<i class="fa-solid fa-bell"></i> เพิ่มการแจ้งเตือนพิเศษ
						</button>
					</div>
				`;
			...
			let html = '<div class="space-y-4 max-h-[60vh] overflow-y-auto px-1 text-left">';
			
			// 1. แสดงการแจ้งเตือนพิเศษพร้อมปุ่มแก้ไขและลบ
			if (dailyNotif.length > 0) {
				html += '<div><h5 class="font-bold text-orange-600 text-sm mb-2 flex items-center"><i class="fa-solid fa-bell mr-2"></i> แจ้งเตือนพิเศษ:</h5><ul class="space-y-2">';
				dailyNotif.forEach(n => {
					let timeBadge = (n.isAllDay === false && n.time) 
						? `<span class="text-[10px] bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full whitespace-nowrap"><i class="fa-regular fa-clock mr-1"></i>${n.time} น.</span>` 
						: `<span class="text-[10px] bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full whitespace-nowrap"><i class="fa-solid fa-sun mr-1"></i>ทั้งวัน</span>`;
			...
			// 2. แสดงกิจกรรมที่นำเข้า (ถ้ามี)
			if (dailyImported.length > 0) {
				html += '<div><h5 class="font-bold text-purple-600 text-sm mb-2 flex items-center"><i class="fa-solid fa-calendar-check mr-2"></i> กิจกรรมที่นำเข้า:</h5><ul class="space-y-2">';
				dailyImported.forEach(ev => {
					html += `
						<li class="flex items-start gap-3 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-100 dark:border-purple-800/50 shadow-sm">
							<i class="fa-solid fa-calendar-day text-purple-500 mt-1"></i>
							<div class="text-left flex-1 min-w-0">
								<div class="font-bold text-gray-800 dark:text-gray-200 text-sm break-words">${escapeHTML(ev.title)}</div>
								<div class="text-xs text-gray-500 dark:text-gray-400 mt-1">🕒 ${ev.allDay ? 'ทั้งวัน' : ev.start.replace('T', ' ')}</div>
							</div>
						</li>
					`;
				});
			...
			// --- สร้าง Header & Footer HTML ---
			const dateObj = new Date(dateStr);
			const formattedDate = dateObj.toLocaleDateString('th-TH', {day: 'numeric', month: 'long', year: 'numeric'});
			
			let headerHtml = `
				<div class="flex justify-between items-center mb-4 w-full border-b border-gray-100 dark:border-gray-700 pb-2">
					<h3 class="text-lg font-bold text-gray-800 dark:text-gray-200">กิจกรรมวันที่ ${formattedDate}</h3>
				</div>
			`;

			let footerHtml = `
				<div class="mt-4 pt-3 border-t border-gray-100 dark:border-gray-700">
					<button id="cal-imported-add-notify-btn" class="w-full bg-orange-500 hover:bg-orange-600 text-white font-medium py-2.5 px-4 rounded-xl shadow-md transition duration-300 flex items-center justify-center gap-2 text-sm">
						<i class="fa-solid fa-bell"></i> เพิ่มการแจ้งเตือนพิเศษ
					</button>
				</div>
			`;
	...
	function showDailyDetails(date) {
        ...
        // [ใหม่] 3. ดึงรายการแจ้งเตือนพิเศษของวันนี้ (Map เก็บ index ไว้สำหรับการอ้างอิงตอนลบ/แก้ไข)
        const dailyNotif = (state.customNotifications || [])
            .map((n, index) => ({ ...n, originalIndex: index }))
            .filter(n => {
                if (typeof isNotificationDue === 'function') return isNotificationDue(n, date);
                return n.date === date;
            });
        ...
        // --- ส่วนแสดงรายการ ---
        if (txsOnDay.length === 0 && recurringOnDay.length === 0 && dailyNotif.length === 0) {
             html += '<p class="text-center text-gray-500 mt-8 mb-8">ไม่มีรายการในวันนี้</p>';
        } else {
            
            // A. แสดงการแจ้งเตือนพิเศษ (ถ้ามี) โชว์ด้านบนสุดเลย
            if (dailyNotif.length > 0) {
                html += `<li class="text-xs font-bold text-orange-500 uppercase tracking-wider mb-2 flex items-center"><i class="fa-solid fa-bell mr-1"></i> แจ้งเตือนพิเศษ</li>`;
                ...
                dailyNotif.forEach(n => {
                    ...
                    let notifHtml = `
                        <div class="flex justify-between items-center bg-orange-50 dark:bg-orange-900/20 p-2.5 rounded-lg border border-orange-100 dark:border-orange-800/50 text-sm shadow-sm w-full gap-2 transition-all">
                            <div class="flex-1 min-w-0">
                                <span class="font-bold text-orange-800 dark:text-orange-200 block truncate">${escapeHTML(n.message)}</span>
                                <div class="mt-1">${timeBadge}</div>
                            </div>
                            <div class="flex items-center gap-1 shrink-0">
                                <button onclick="editNotifyFromCal('${date}', ${n.originalIndex})" class="text-blue-500 hover:text-blue-700 bg-blue-100 dark:bg-blue-900/40 p-2 rounded-md transition hover:scale-110" title="แก้ไข">
                                    <i class="fa-solid fa-pen text-xs"></i>
                                </button>
                                <button onclick="deleteNotifyFromCal(${n.originalIndex})" class="text-red-500 hover:text-red-700 bg-red-100 dark:bg-red-900/40 p-2 rounded-md transition hover:scale-110" title="ลบ">
                                    <i class="fa-solid fa-trash text-xs"></i>
                    ...
                    `;
                    html += `<li class="pb-2 mt-1">${notifHtml}</li>`;
                });
                
                if (txsOnDay.length > 0 || recurringOnDay.length > 0) {
                    html += `<li class="border-t border-dashed border-gray-300 mt-2 pt-2"></li>`;
                }
            }

            // B. แสดงรายการที่บันทึกแล้ว
            txsOnDay.forEach(tx => {
                let txHtml = '';
            ...
            // C. แสดงรายการ Recurring (รอจ่าย/รอรับ)
            if (recurringOnDay.length > 0) {
                if (txsOnDay.length > 0 || dailyNotif.length > 0) {
    ...
    async function renderSettings() {
		...
		// +++ สวิตช์แสดงคำแนะนำปุ่มไมค์ +++
		const toggleVoiceTooltip = document.getElementById('toggle-voice-tooltip');
		if (toggleVoiceTooltip) {
			toggleVoiceTooltip.checked = state.showVoiceTooltip;
			toggleVoiceTooltip.addEventListener('change', async (e) => {
				window.appVibrate(50);  // <-- เพิ่มตรงนี้
				const isChecked = e.target.checked;
				state.showVoiceTooltip = isChecked;
				await dbPut(STORE_CONFIG, { key: 'showVoiceTooltip', value: isChecked });

				// จัดการกับ tooltip ที่กำลังแสดง
				if (!isChecked) {
					// ปิด: หยุด loop tooltip และซ่อน tooltip ที่แสดงอยู่
					if (window._smartVoiceControls) {
						window._smartVoiceControls.stopTooltipLoop();
						window._smartVoiceControls.hideTooltip();
					}
				} else {
					// เปิด: ถ้าปุ่มกำลังกระดิกอยู่ ให้เริ่ม loop tooltip อีกครั้ง
					const btn = document.getElementById('smart-voice-btn');
					if (btn && btn.classList.contains('smart-voice-wiggle') && window._smartVoiceControls) {
						window._smartVoiceControls.startTooltipLoop();
					}
				}

				showToast(isChecked ? 'เปิดคำแนะนำปุ่มไมค์แล้ว' : 'ปิดคำแนะนำปุ่มไมค์แล้ว', 'success');
			});
		}
		...
		// +++ เพิ่มสองบรรทัดนี้ +++
		const lastManualSpan = document.getElementById('last-manual-backup-time');
		const lastCloudSpan = document.getElementById('last-cloud-sync-time');

		if (toggleAutoBackup && freqSelect && lastBackupSpan) {
			// Clone elements to remove old listeners
			const newToggle = toggleAutoBackup.cloneNode(true);
			toggleAutoBackup.parentNode.replaceChild(newToggle, toggleAutoBackup);
			const newFreq = freqSelect.cloneNode(true);
			freqSelect.parentNode.replaceChild(newFreq, freqSelect);

			// Re-assign variables to new elements
			const finalToggle = newToggle;
			const finalFreq = newFreq;

			// ตั้งค่าตาม state
			finalToggle.checked = state.autoBackup.enabled;
			finalFreq.value = state.autoBackup.frequency;

			// บังคับให้ select ใช้งานได้เสมอ
			finalFreq.disabled = false;

			// แสดงเวลาสำรองครั้งล่าสุด (auto)
			if (state.autoBackup.lastBackup) {
				const d = new Date(state.autoBackup.lastBackup);
				lastBackupSpan.textContent = d.toLocaleString('th-TH');
			} else {
				lastBackupSpan.textContent = 'ไม่เคย';
			}

			// +++ แสดงเวลาสำรอง manual และ cloud +++
			if (lastManualSpan) {
				lastManualSpan.textContent = state.manualBackupLastTime
					? new Date(state.manualBackupLastTime).toLocaleString('th-TH')
					: 'ไม่เคย';
			}
			if (lastCloudSpan) {
				lastCloudSpan.textContent = state.cloudSyncLastTime
					? new Date(state.cloudSyncLastTime).toLocaleString('th-TH')
					: 'ไม่เคย';
			}

			// Event เมื่อเปลี่ยนสถานะเปิด/ปิด
			finalToggle.addEventListener('change', async (e) => {
				const enabled = e.target.checked;
				state.autoBackup.enabled = enabled;
				finalFreq.disabled = false; // ยืนยันไม่ disable
				await saveAutoBackupSettings();

				if (enabled) {
					const freqMap = { 'daily': 'รายวัน', 'weekly': 'รายสัปดาห์', 'monthly': 'รายเดือน' };
					const freqText = freqMap[state.autoBackup.frequency] || state.autoBackup.frequency;
					showToast(`✅ เปิดสำรองข้อมูลอัตโนมัติ (${freqText})`, 'success');
				} else {
					showToast('❌ ปิดสำรองข้อมูลอัตโนมัติ', 'info');
				}
			});

			// Event เมื่อเปลี่ยนความถี่
			finalFreq.addEventListener('change', async (e) => {
				state.autoBackup.frequency = e.target.value;
				await saveAutoBackupSettings();

				if (state.autoBackup.enabled) {
					const freqMap = { 'daily': 'รายวัน', 'weekly': 'รายสัปดาห์', 'monthly': 'รายเดือน' };
					const freqText = freqMap[state.autoBackup.frequency] || state.autoBackup.frequency;
					showToast(`📅 เปลี่ยนความถี่เป็น ${freqText}`, 'info');
				}
			});
		}
	...
	async function checkAndRunAutoBackup() {
		...
		// ฟังก์ชันแปลงวันที่เป็น YYYY-MM-DD แบบ local
		...
		if (!lastBackup) {
			...
			const lastDateStr = formatLocalDate(lastBackup);
			const todayStr = formatLocalDate(now);

			switch (state.autoBackup.frequency) {
				case 'daily':
		...
		if (shouldBackup) {
			...
			const lastBackupSpan = document.getElementById('last-auto-backup-time');
			if (lastBackupSpan) lastBackupSpan.textContent = now.toLocaleString('th-TH');
		}
	}
	
	async function runAutoBackup() {
		// ป้องกันการทำงานซ้อน
		if (window._isAutoBackupRunning) return;
		window._isAutoBackupRunning = true;

		try {
			// รวบรวมข้อมูลจากทุก store (เหมือนเดิม)
			const accounts = await dbGetAll(STORE_ACCOUNTS);
			const transactions = await dbGetAll(STORE_TRANSACTIONS);
			
			const incomeCats = (await dbGet(STORE_CATEGORIES, 'income'))?.items || [];
			const expenseCats = (await dbGet(STORE_CATEGORIES, 'expense'))?.items || [];
			
			const frequentItems = (await dbGetAll(STORE_FREQUENT_ITEMS)).map(item => item.name);
			const autoCompleteList = await dbGetAll(STORE_AUTO_COMPLETE);
			const recurringRules = await dbGetAll(STORE_RECURRING) || [];
			const budgets = await dbGetAll(STORE_BUDGETS) || [];
			const voiceCommands = await dbGetAll(STORE_VOICE_COMMANDS) || [];
			const icsImports = await dbGetAll(STORE_ICS_IMPORTS) || [];
			const importedEvents = await dbGetAll(STORE_IMPORTED_EVENTS) || [];
			
			// Config ต่างๆ
			const password = (await dbGet(STORE_CONFIG, 'password'))?.value || null;
			const autoLockTimeout = (await dbGet(STORE_CONFIG, AUTOLOCK_CONFIG_KEY))?.value || 0;
			const isDarkMode = (await dbGet(STORE_CONFIG, DARK_MODE_CONFIG_KEY))?.value || false;
			const autoConfirmPassword = (await dbGet(STORE_CONFIG, AUTO_CONFIRM_CONFIG_KEY))?.value || false;
			const showBalanceCard = (await dbGet(STORE_CONFIG, 'showBalanceCard'))?.value || false;
			const collapsePreferences = (await dbGet(STORE_CONFIG, 'collapse_preferences'))?.value || {};
			const notifySettings = (await dbGet(STORE_CONFIG, 'notification_settings'))?.value || { scheduled: true, recurring: true, budget: true };
			const ignoredNotifications = (await dbGet(STORE_CONFIG, 'ignored_notifications'))?.value || [];
			const customNotificationsList = (await dbGet(STORE_CONFIG, 'custom_notifications_list'))?.value || [];
			const notificationHistory = (await dbGet(STORE_CONFIG, 'notification_history'))?.value || [];
			const mobileMenuStyle = (await dbGet(STORE_CONFIG, 'mobileMenuStyle'))?.value || 'bottom';
			const lineUserIdsList = (await dbGet(STORE_CONFIG, 'lineUserIds_List'))?.value || [];
			const lineNotifyActions = state.lineNotifyActions || { add: true, edit: true, delete: true };
			...
			let baseName = 'autobackup';
			if (window.auth && window.auth.currentUser && window.auth.currentUser.email) {
				const email = window.auth.currentUser.email;
				const atIndex = email.indexOf('@');
				let localPart = atIndex > 0 ? email.substring(0, atIndex) : email;
				baseName = localPart + '_autobackup';
			}
			const version = typeof APP_VERSION !== 'undefined' ? APP_VERSION : 'unknown';
			const filename = `${baseName}_${version}_${dateStr}_${timeStr}.json`;
			...
			// --- เปลี่ยนจาก Toast เป็น SweetAlert2 Popup ---
			// แปลงความถี่เป็นข้อความภาษาไทย
			const freqText = {
				'daily': 'รายวัน',
				'weekly': 'รายสัปดาห์',
				'monthly': 'รายเดือน'
			}[state.autoBackup.frequency] || 'อัตโนมัติ';
			...
			setTimeout(() => {
				Swal.fire({
					...
					html: `
						<div class="text-left">
							<p><strong>ประเภท:</strong> ${freqText}</p>
							<p><strong>ไฟล์:</strong> ${filename}</p>
							<p><strong>เวลา:</strong> ${now.toLocaleString('th-TH')}</p>
						</div>
					`,
					confirmButtonText: 'ตกลง',
					confirmButtonColor: '#10b981',
					allowOutsideClick: false,   // ห้ามปิดโดยคลิกด้านนอก
					allowEscapeKey: false,      // ห้ามปิดโดยกด ESC
					customClass: {
						popup: state.isDarkMode ? 'swal2-popup' : ''
					},
					background: state.isDarkMode ? '#1a1a1a' : '#fff',
			...
			// หลังจาก Backup เสร็จ (ใน runAutoBackup)
			if (document.getElementById('page-settings') && !document.getElementById('page-settings').classList.contains('hidden')) {
				// ถ้าผู้ใช้กำลังอยู่ในหน้า Settings ให้รีเฟรชเฉพาะส่วนนั้น
				const lastBackupSpan = document.getElementById('last-auto-backup-time');
				if (lastBackupSpan) {
					lastBackupSpan.textContent = new Date().toLocaleString('th-TH');
				}
			}
			...
			// ถ้าเกิด error ก็แจ้งเตือนด้วย Popup เช่นกัน
			Swal.fire({
				icon: 'error',
				title: '❌ สำรองข้อมูลอัตโนมัติไม่สำเร็จ',
				text: err.message || 'กรุณาลองอีกครั้ง',
				confirmButtonText: 'ตกลง'
			});
		} finally {
			window._isAutoBackupRunning = false;
		}
	}
    ...
    function openAccountModal(accountId = null, closeOnly = false) {
		...
		if (balDisplay) {
			balDisplay.innerText = typeof formatCurrency === 'function' ? formatCurrency(currentBal) : currentBal;
			balDisplay.className = `text-xl font-bold ${currentBal >= 0 ? 'text-blue-600' : 'text-red-600'} dark:${currentBal >= 0 ? 'text-blue-400' : 'text-red-400'}`;
			balDisplay.dataset.rawValue = currentBal; 
		}

		// รีเซ็ตช่องกรอก Smart Calculator (ระบบใหม่)
		const smartInputNew = document.getElementById('smart-input-new');
		const smartInputDesc = document.getElementById('smart-input-desc');
		const smartDisplayDiff = document.getElementById('smart-display-diff');
		const hiddenAmt = document.getElementById('adjust-tx-amount');
		const hiddenType = document.getElementById('adjust-tx-type');
		const hiddenDesc = document.getElementById('adjust-tx-desc');

		if (smartInputNew) {
			// ใช้ cloneNode เพื่อล้าง Event Listener เก่าทิ้ง ป้องกันการคำนวณซ้อน
			const newSmartInput = smartInputNew.cloneNode(true);
			smartInputNew.parentNode.replaceChild(newSmartInput, smartInputNew);
			newSmartInput.value = '';
			
			// คำนวณส่วนต่างอัตโนมัติเมื่อพิมพ์ตัวเลข
			newSmartInput.addEventListener('input', () => {
				const current = parseFloat(balDisplay.dataset.rawValue || 0);
				const newVal = parseFloat(newSmartInput.value);
				
				if (isNaN(newVal)) {
					if (smartDisplayDiff) smartDisplayDiff.classList.add('hidden');
					if (hiddenAmt) hiddenAmt.value = '';
					return;
				}

				const diff = newVal - current;
				if (smartDisplayDiff) smartDisplayDiff.classList.remove('hidden');

				const formatMoney = (val) => typeof formatCurrency === 'function' ? formatCurrency(val) : parseFloat(val).toLocaleString('th-TH', {minimumFractionDigits: 2}) + ' ฿';

				if (diff > 0) {
					if (smartDisplayDiff) {
						smartDisplayDiff.innerHTML = `<i class="fa-solid fa-arrow-trend-up mr-1"></i> ปรับยอดเพิ่ม ${formatMoney(diff)} <span class="text-xs font-normal">(บันทึกเป็นรายรับ)</span>`;
						smartDisplayDiff.className = 'mb-3 p-3 rounded-xl text-center text-sm font-semibold text-green-700 bg-green-100 dark:text-green-400 dark:bg-green-900/30 border border-green-200 dark:border-green-800 transition-all';
					}
					if (hiddenType) hiddenType.value = 'income';
					if (hiddenAmt) hiddenAmt.value = Math.abs(diff);
				} else if (diff < 0) {
					if (smartDisplayDiff) {
						smartDisplayDiff.innerHTML = `<i class="fa-solid fa-arrow-trend-down mr-1"></i> ปรับยอดลด ${formatMoney(Math.abs(diff))} <span class="text-xs font-normal">(บันทึกเป็นรายจ่าย)</span>`;
						smartDisplayDiff.className = 'mb-3 p-3 rounded-xl text-center text-sm font-semibold text-red-700 bg-red-100 dark:text-red-400 dark:bg-red-900/30 border border-red-200 dark:border-red-800 transition-all';
					}
					if (hiddenType) hiddenType.value = 'expense';
					if (hiddenAmt) hiddenAmt.value = Math.abs(diff);
				} else {
					if (smartDisplayDiff) {
						smartDisplayDiff.innerHTML = 'ยอดเงินตรงกัน (ไม่มีการเปลี่ยนแปลง)';
						smartDisplayDiff.className = 'mb-3 p-3 rounded-xl text-center text-sm font-semibold text-gray-600 bg-gray-100 dark:text-gray-400 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-all';
					}
					if (hiddenAmt) hiddenAmt.value = '';
				}
			});
		...
		if (smartInputDesc) {
			const newSmartDesc = smartInputDesc.cloneNode(true);
			smartInputDesc.parentNode.replaceChild(newSmartDesc, smartInputDesc);
			newSmartDesc.value = '';
			newSmartDesc.addEventListener('input', () => {
				 if (hiddenDesc) hiddenDesc.value = newSmartDesc.value;
			});
		}

		if (smartDisplayDiff) {
			smartDisplayDiff.classList.add('hidden');
			smartDisplayDiff.innerHTML = '';
		}
		if (hiddenAmt) hiddenAmt.value = '';
   ...
   async function handleDeleteClick(buttonEl) {
    ...
    async function handleManagePassword() {
		if (state.password) {
				...
				const { value: formValues } = await Swal.fire({
					...
					html: `
						<div class="mb-3 text-left">
							...
							<select id="swal-password-type" class="swal2-input" style="margin:0; width:100%">
								<option value="pin" ${state.passwordType === 'pin' ? 'selected' : ''}>PIN (ตัวเลขเท่านั้น)</option>
								<option value="text" ${state.passwordType === 'text' ? 'selected' : ''}>ข้อความ (ตัวอักษร+ตัวเลข)</option>
							</select>
						</div>
						<div class="text-left">
							<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">รหัสผ่านใหม่</label>
							<input type="password" id="swal-password" class="swal2-input" placeholder="รหัสผ่านใหม่" style="margin:0 auto; width:100%; max-width:80%;">
						</div>
					`,
					focusConfirm: false,
					showCancelButton: true,
					didOpen: () => {
						// ถ้าเป็น PIN ให้ปรับ inputmode ของช่องรหัสผ่านใหม่ (เผื่อ HTML string ไม่เวิร์ก)
						const typeSelect = document.getElementById('swal-password-type');
						const pwdInput = document.getElementById('swal-password');
						if (typeSelect && pwdInput) {
							const updateInputMode = () => {
								if (typeSelect.value === 'pin') {
									pwdInput.setAttribute('inputmode', 'numeric');
									pwdInput.setAttribute('pattern', '\\d*');
								} else {
							...
							// ตั้งค่าตอนแรก
							updateInputMode();
							// เปลี่ยนตามเมื่อเลือกประเภท
							typeSelect.addEventListener('change', updateInputMode);
						}
					},
					preConfirm: () => {
						const pwdType = document.getElementById('swal-password-type').value;
						const pwd = document.getElementById('swal-password').value;
						if (!pwd) {
							Swal.showValidationMessage('กรุณากรอกรหัสผ่านใหม่');
							return false;
						}
						if (pwdType === 'pin' && !/^\d+$/.test(pwd)) {
							Swal.showValidationMessage('PIN ต้องเป็นตัวเลขเท่านั้น');
							return false;
						}
						return { type: pwdType, password: pwd };
					}
				});

				if (formValues) {
					const { type, password } = formValues;

					// ขั้นตอนยืนยันรหัสผ่าน
					const { value: confirmResult } = await Swal.fire({
						title: 'ยืนยันรหัสผ่านใหม่',
						html: `
							<div class="text-left">
								<input type="password" id="swal-confirm-password" class="swal2-input" placeholder="กรอกรหัสผ่านใหม่อีกครั้ง" style="margin:0 auto; width:100%; max-width:80%;">
							</div>
						`,
						focusConfirm: false,
						showCancelButton: true,
						didOpen: () => {
							// ตั้งค่า inputmode ตามประเภทที่เลือกไว้
							const confirmInput = document.getElementById('swal-confirm-password');
							if (confirmInput && type === 'pin') {
								confirmInput.setAttribute('inputmode', 'numeric');
								confirmInput.setAttribute('pattern', '\\d*');
							} else if (confirmInput) {
								confirmInput.removeAttribute('inputmode');
								confirmInput.removeAttribute('pattern');
							}
						},
						preConfirm: () => {
							const confirmPwd = document.getElementById('swal-confirm-password').value;
							if (!confirmPwd) {
								Swal.showValidationMessage('กรุณากรอกรหัสผ่านยืนยัน');
								return false;
							}
							if (type === 'pin' && !/^\d+$/.test(confirmPwd)) {
								Swal.showValidationMessage('PIN ต้องเป็นตัวเลขเท่านั้น');
								return false;
							}
					...
					if (confirmResult) {
						...
						await dbPut(STORE_CONFIG, { key: 'passwordType', value: type });
						state.password = hashedNewPassword;
						state.passwordType = type;
						Swal.fire('สำเร็จ!', 'เปลี่ยนรหัสผ่านเรียบร้อย', 'success');
						addActivityLog('🔑 เปลี่ยนรหัสผ่าน', `ประเภท: ${type === 'pin' ? 'PIN' : 'ข้อความ'}`, 'fa-key', 'text-blue-600');
						resetAutoLockTimer();
					}
				}
			} else if (action === false) {
				// ลบรหัสผ่าน (เหมือนเดิม)
				Swal.fire({
			...
			const { value: formValues } = await Swal.fire({
				...
				html: `
					<div class="mb-3 text-left">
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ประเภท</label>
						<select id="swal-password-type" class="swal2-input" style="margin:0; width:100%">
							<option value="pin">PIN (ตัวเลขเท่านั้น)</option>
							<option value="text">ข้อความ (ตัวอักษร+ตัวเลข)</option>
						</select>
					</div>
					<div class="text-left">
						<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">รหัสผ่าน</label>
						<input type="password" id="swal-password" class="swal2-input" placeholder="รหัสผ่าน" style="margin:0 auto; width:100%; max-width:80%;">
					</div>
				`,
				focusConfirm: false,
				showCancelButton: true,
			...
			if (formValues) {
				...
				// ขั้นตอนยืนยันรหัสผ่าน (เหมือนกรณีเปลี่ยน)
				const { value: confirmResult } = await Swal.fire({
					title: 'ยืนยันรหัสผ่านใหม่',
					html: `
						<div class="text-left">
				...
				if (confirmResult) {
					...
					addActivityLog('🔐 ตั้งรหัสผ่าน', `ประเภท: ${type === 'pin' ? 'PIN' : 'ข้อความ'}`, 'fa-key', 'text-green-600');
					resetAutoLockTimer();
				}
			}
		}
			...
			if (isConfirmed) {
				try {
					...
					const backupState = {
						...
						icsImports: await dbGetAll(STORE_ICS_IMPORTS) || [],
						importedEvents: await dbGetAll(STORE_IMPORTED_EVENTS) || [],
					...
					// +++ บันทึกเวลา manual backup +++
					state.manualBackupLastTime = new Date().toISOString();
					await dbPut(STORE_CONFIG, { key: 'manualBackupLastTime', value: state.manualBackupLastTime });
					
					// +++ อัปเดต UI ทันที (ถ้ามี element นี้อยู่) +++
					const manualSpan = document.getElementById('last-manual-backup-time');
					if (manualSpan) {
						manualSpan.textContent = new Date(state.manualBackupLastTime).toLocaleString('th-TH');
					}
					// +++++++++++++++++++++++++++++
			...
			if (result.isConfirmed) {
				...
				try {
					const collections = [
						...
						STORE_RECURRING, STORE_BUDGETS, STORE_ICS_IMPORTS, STORE_IMPORTED_EVENTS
					];
					...
					// +++ บันทึกเวลา cloud sync +++
					const now = new Date();   // ✅ เพิ่มบรรทัดนี้
					state.cloudSyncLastTime = now.toISOString();
					await updateCloudSyncTimeInDB(state.cloudSyncLastTime);
					...
					const cloudSpan = document.getElementById('last-cloud-sync-time');
					if (cloudSpan) {
						cloudSpan.textContent = new Date(state.cloudSyncLastTime).toLocaleString('th-TH');
					}
					// +++++++++++++++++++++++++++++
    ...
    async function handleClearAll() {
        ...
        const mapText = {
            'local': 'ในเครื่องทั้งหมด ไม่สามารถกูคืนได้ (ข้อมูลบน Cloud จะยังอยู่หากเข้าสู่ระบบไว้)',
		...
		async function promptForPassword(promptTitle = 'กรุณาใส่รหัสผ่าน') {
			// ถ้าไม่มีรหัสผ่านในระบบ ให้ผ่านทันที
			if (state.password === null) return true;

			// --- ขั้นตอนที่ 1: ถ้ามีการตั้ง Biometric ไว้ ให้ลองสแกนก่อน ---
			if (state.biometricId) {
				try {
			...
			// --- ขั้นตอนที่ 2: แสดงฟอร์มป้อนรหัสผ่าน (โค้ดเดิม) ---
			...
			`;
			if (state.passwordType === 'pin') {
				htmlContent += ` inputmode="numeric" pattern="\\d*"`;
			}
			htmlContent += `>`;
				...
				showDemoModal(
					...
					`<div class="text-center space-y-4">
						...
						<div class="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-xl text-left">
							...
							<p class="text-sm ml-6 text-gray-600 dark:text-gray-400">กดที่รูปคอมพิวเตอร์ลูกศรชี้ลง ในช่อง URL → “กดติดตั้ง”</p>
							...
							<p class="font-bold flex items-center gap-2 mt-2 text-purple-700 dark:text-purple-300"><i class="fa-solid fa-download"></i> หรือกดติดตั้งด้วยปุ่มด้านล่าง </p>
						</div>
						<button id="btn-pwa-install" class="w-full py-3 px-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-xl font-bold shadow-md transition-all active:scale-95 flex items-center justify-center gap-2">
								<i class="fa-solid fa-download"></i>
								<span>กดเพื่อติดตั้งแอป</span>
						</button>
					</div>`
				);
			};
			
			// ฟังก์ชันสำหรับเรียกหน้าต่าง Welcome Modal โดยไม่กระทบกับระบบการบันทึกสถานะ (จำลองการเปิด)
			window.showWelcomeDemo = function() {
			...
			// ฟังก์ชันสร้างหน้าต่าง Demo สำหรับสอนการตั้งรหัสผ่านใหม่ (แทนที่ของเดิมใน script.js)
			window.showNewPasswordDemo = function() {
				if (typeof Swal !== 'undefined') {
					Swal.fire({
						...
						html: `<div class="text-center space-y-4">
							...
							<p class="text-gray-700 dark:text-gray-300">ระบบไม่มีรหัสผ่านเริ่มต้นให้ คุณต้องเป็นผู้ตั้งรหัสผ่านเอง</p>
							<p class="text-sm text-gray-500 dark:text-gray-400">สามารถตั้งได้ตั้งแต่เปิดแอปครั้งแรก หรือตั้งภายหลังในหน้าตั้งค่า</p>
							<div class="bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded-xl text-left text-sm">
								<p class="font-bold text-yellow-800 dark:text-yellow-400 flex items-center gap-2"><i class="fa-solid fa-lightbulb"></i> วิธีตั้งค่า / เปลี่ยนรหัสผ่าน</p>
								<p class="text-gray-600 dark:text-gray-400 ml-6">ไปที่เมนู <span class="bg-purple-100 dark:bg-purple-900/30 px-2 py-0.5 rounded text-purple-700 dark:text-purple-400">ตั้งค่า → จัดการรหัสผ่าน</span></p>
							</div>
							<button onclick="Swal.close(); showPage('page-settings');" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-full text-sm font-bold shadow-md transition">
								<i class="fa-solid fa-gear mr-1"></i> ไปที่เมนูตั้งค่า
							</button>
						</div>`,
						icon: 'info',
						confirmButtonText: 'ปิด',
			...
			function checkNotifications() {
				...
				todaysTransactions.forEach(tx => {
					...
					if (state.notifySettings.recurring && tx.id.startsWith('tx-rec-')) {
						const ruleId = tx.ruleId;   // ดึง ruleId จาก transaction
						if (!ruleId) return;      // ถ้าไม่มี (transaction เก่า) ให้ข้ามไป

						const alertId = `recurring_${ruleId}`;   // ID คงที่สำหรับกฎนี้

						// ตรวจสอบว่าเคย ignore หรือไม่
						if (state.ignoredNotifications.includes(alertId)) return;

						alerts.push({
							id: alertId,            // ใช้ ID คงที่
							title: 'รายการประจำถึงกำหนด',
							message: `${tx.name} (${formatCurrency(tx.amount)})`,
							icon: 'fa-rotate',
							ruleId: ruleId           // เก็บไว้ใช้ถ้าจำเป็น
						});
					}
		...
		function ensureTooltip() {
			if (!wiggleTooltip) {
				wiggleTooltip = document.createElement('div');
				wiggleTooltip.id = 'smart-voice-wiggle-tooltip';
				wiggleTooltip.style.position = 'fixed';
				wiggleTooltip.style.backgroundColor = 'rgba(0,0,0,0.8)';
				wiggleTooltip.style.color = 'white';
				wiggleTooltip.style.padding = '8px 16px';
				wiggleTooltip.style.borderRadius = '20px';
				wiggleTooltip.style.fontSize = '14px';
				wiggleTooltip.style.fontWeight = 'bold';
				wiggleTooltip.style.whiteSpace = 'nowrap';
				wiggleTooltip.style.boxShadow = '0 4px 10px rgba(0,0,0,0.3)';
				wiggleTooltip.style.zIndex = '10000';
				wiggleTooltip.style.pointerEvents = 'none';
				wiggleTooltip.style.transition = 'opacity 0.2s';
				document.body.appendChild(wiggleTooltip);
			}
			return wiggleTooltip;
		}

		// ฟังก์ชันแสดง tooltip พร้อมข้อความสุ่ม ปรับตำแหน่งไม่ให้ล้นจอ
		function showTooltipWithRandomMessage() {
			const btn = document.getElementById('smart-voice-btn');
			if (!btn) return;

			const tooltip = ensureTooltip();
			const randomIndex = Math.floor(Math.random() * WIGGLE_MESSAGES.length);
			const msg = WIGGLE_MESSAGES[randomIndex];
		...
		function hideTooltip() {
			if (wiggleTooltip) {
				wiggleTooltip.style.opacity = '0';
				setTimeout(() => {
					if (wiggleTooltip && wiggleTooltip.style.opacity === '0') {
						wiggleTooltip.style.display = 'none';
					}
				}, 200);
			}
			if (tooltipHideTimer) {
		...
		function stopTooltipLoop() {
			if (tooltipShowInterval) {
				clearInterval(tooltipShowInterval);
				tooltipShowInterval = null;
			}
			hideTooltip();
		}

		// ฟังก์ชันเริ่มจับเวลาและสั่งกระดิกเมื่อครบกำหนด
		...
		// ฟังก์ชันหยุดการกระดิกและล้าง timer
		function stopWiggle() {
			if (wiggleTimer) {
		...
		function initDraggableVoiceButton() {
			...
			// ตัวแปรสำหรับการกระดิกและ tooltip
			let wiggleTimer = null;
			let tooltipHideTimer = null;
			let tooltipShowInterval = null;
			let wiggleTooltip = null;
			...
			function showTooltipWithRandomMessage() {
				// +++ ตรวจสอบ state.showVoiceTooltip ก่อนแสดง +++
				if (!state.showVoiceTooltip) return;
			...
			function startTooltipLoop() {
				if (!state.showVoiceTooltip) return; // ปิดอยู่ไม่ต้องเริ่ม
				if (tooltipShowInterval) clearInterval(tooltipShowInterval);
				showTooltipWithRandomMessage();
				tooltipShowInterval = setInterval(() => {
					showTooltipWithRandomMessage();
			...
			function startWiggleTimer() {
				...
				wiggleTimer = setTimeout(() => {
					if (btn) {
						btn.classList.add('smart-voice-wiggle');
						startTooltipLoop(); // เริ่ม loop tooltip (ถ้าเปิดอยู่)
					}
				}, WIGGLE_DELAY);
			}
			...
			// เริ่มจับเวลากระดิกครั้งแรก
			startWiggleTimer();

			// --- ตัวแปรและฟังก์ชันสำหรับการลาก ---
			let isDragging = false;
			let dragStartX, dragStartY, dragStartLeft, dragStartTop;
			let dragTimeout;

			const startDrag = (e) => {
				stopWiggle(); // หยุดกระดิกขณะลาก
			...
			const onDrag = (e) => {
				...
				if (!isDragging && (Math.abs(dx) > 5 || Math.abs(dy) > 5)) {
					...
					window.appVibrate([20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20, 50, 20]);
				}
			...
			const stopDrag = (e) => {
				...
				if (isDragging) {
					...
					if (left && top) {
						localStorage.setItem('smartVoicePos', JSON.stringify({ left, top }));
					}
				...
				isDragging = false;
				startWiggleTimer(); // เริ่มจับเวลากระดิกใหม่เมื่อปล่อย
			};
			...
			// +++ Expose ฟังก์ชันควบคุมผ่าน window เพื่อให้เรียกจาก renderSettings ได้ +++
			window._smartVoiceControls = {
				startTooltipLoop,
				stopTooltipLoop,
				hideTooltip
			};
			...
			/**
			 * ค้นหาคำสั่งที่ผู้ใช้สอนไว้ รองรับแบบ prefix (พูดคำสั่งต้นประโยค)
			 * @param {string} spokenText - ข้อความที่ได้จาก Speech Recognition
			 * @returns {Promise<{command: object, remainingText: string}|null>}
			 */
			async function findLearnedCommand(spokenText) {
				try {
					...
					const politeSuffixes = ['ครับ', 'ค่ะ', 'จ้ะ', 'จ้า', 'นะ', 'หน่อย', 'ที', 'น่ะ', 'ละ', 'หละ', 'จั๊บ', 'เด้อ', 'เถอะ', 'ซิ'];
					for (const suf of politeSuffixes) {
						if (cleanSpoken.endsWith(suf)) {
							cleanSpoken = cleanSpoken.slice(0, -suf.length).trim();
							break;
						}
					}

					// 1. Exact match (ตรงทั้งหมด)
					const exactMatch = allCommands.find(cmd => {
						let cleanCmd = cmd.command.trim().toLowerCase();
						for (const suf of politeSuffixes) {
							if (cleanCmd.endsWith(suf)) {
								cleanCmd = cleanCmd.slice(0, -suf.length).trim();
								break;
							}
						}
						return cleanCmd === cleanSpoken;
					});
					if (exactMatch) {
						exactMatch.useCount = (exactMatch.useCount || 0) + 1;
						await dbPut(STORE_VOICE_COMMANDS, exactMatch);
						return { command: exactMatch, remainingText: '' };
					}

					// 2. Prefix match (จับคู่ส่วนต้น)
					const sortedCommands = [...allCommands].sort((a, b) => b.command.length - a.command.length);
					for (const cmd of sortedCommands) {
						...
						if (cleanSpoken.startsWith(cleanCmd)) {
							const remaining = cleanSpoken.slice(cleanCmd.length).trim();
							cmd.useCount = (cmd.useCount || 0) + 1;
							await dbPut(STORE_VOICE_COMMANDS, cmd);
							return { command: cmd, remainingText: remaining };
						}
					}

					// 3. Token overlap (เดิม) ใช้เมื่อไม่เจอ prefix
					const spokenTokens = cleanSpoken.split(/\s+/).filter(t => t.length > 1);
					...
					for (const cmd of allCommands) {
						...
						const cmdTokens = cleanCmd.split(/\s+/).filter(t => t.length > 1);
					...
					if (bestMatch) {
						...
						return { command: bestMatch, remainingText: '' };
					}
			...
			/**
			 * ดำเนินการตามคำสั่งที่เรียนรู้
			 * @param {object} cmd - ข้อมูลคำสั่งที่เรียนรู้
			 * @param {string} remainingText - ข้อความที่เหลือหลังจากตัดคำสั่งออก (ถ้ามี)
			 */
			async function executeLearnedCommand(cmd, remainingText = '') {
				...
				const applyTimeFromRemaining = () => {
					...
					if (timeInfo.mode === 'custom' && timeInfo.startDt && timeInfo.endDt) {
						...
						const end = new Date(timeInfo.year, timeInfo.month + 1, 0);
						const formatDate = (d) => {
						...
						document.getElementById('adv-filter-start').value = formatDate(start);
						document.getElementById('adv-filter-end').value = formatDate(end);
						return true;
					}
					return false;
				};
				...
				switch (cmd.action) {
						...
						setTimeout(() => {
							...
							if (remainingText) {
								...
								if (timeInfo.mode === 'custom' && timeInfo.startDt && timeInfo.endDt) {
									...
									timeLabel = `เดือน${monthNames[timeInfo.month]} พ.ศ. ${timeInfo.year + 543}`;
								} else {
									applyCustomPeriod(remainingText);
									timeLabel = remainingText;
								}
							}
							...
							setTimeout(() => {
								...
								if (filtered.length === 0) {
									let msg = `ไม่พบรายการที่ตรงกับ "${keyword}"`;
									if (timeLabel) msg += ` ใน${timeLabel}`;
									speak(msg);
									return;
								}
								...
								if (incomeCount > 0 && expenseCount > 0) {
									...
									summary += ` คงเหลือสุทธิ ${formatCurrency(net)}`;
								} else if (incomeCount > 0) {
									summary += ` รายรับ ${incomeCount} รายการ รวม ${formatCurrency(incomeSum)}`;
								} else if (expenseCount > 0) {
									summary += ` รายจ่าย ${expenseCount} รายการ รวม ${formatCurrency(expenseSum)}`;
								}

								if (transferCount > 0) {
									summary += ` (โอนย้าย ${transferCount} รายการ รวม ${formatCurrency(transferSum)})`;
								}
					...
					case 'filterByType':
						...
						setTimeout(() => {
							const typeSelect = document.getElementById('adv-filter-type');
							if (typeSelect) {
								typeSelect.value = cmd.filterType;
							...
							setTimeout(() => {
								const filtered = getCurrentFilteredTransactions();
								const typeName = cmd.filterType === 'income' ? 'รายรับ' : 'รายจ่าย';
								if (filtered.length === 0) {
									let msg = `ไม่พบรายการ${typeName}`;
									if (timeLabel) msg += ` ใน${timeLabel}`;
									speak(msg);
									return;
								}
								...
								let msg = `พบ ${filtered.length} รายการ${typeName}`;
								if (timeLabel) msg += ` ใน${timeLabel}`;
								msg += ` รวม ${formatCurrency(total)}`;
								speak(msg);
							}, 800);
						}, 300);
						break;
					...
					case 'applyTimeFilter':
						if (cmd.period) {
							applyCustomPeriod(cmd.period);
							speak(`รับทราบ แสดงข้อมูล${cmd.period}`);
						}
						break;
			...
			async function processGlobalCommand(text) {
				...
				const learnedResult = await findLearnedCommand(text);
				if (learnedResult) {
					const { command, remainingText } = learnedResult;
					console.log('🎓 Found learned command:', command, 'Remaining:', remainingText);
					if (typeof executeLearnedCommand === 'function') {
						await executeLearnedCommand(command, remainingText);
					}
					return;
				}
			...
			async function calculateVoiceFinanceContext(intent, specificCategory, timeInfo, isMin) {
				...
				if (intent === 'summary') {
					...
					return `สำหรับ${timeInfo.label} มีรายจ่ายรวม ${expense.toLocaleString()} บาท รายรับ ${income.toLocaleString()} บาท คงเหลือสุทธิ ${balance.toLocaleString()} บาท`;
				}

				if (intent === 'extreme_expense') {
					let expenses = filteredTxs.filter(t => t.type === 'expense');
					if(expenses.length === 0) return `${timeInfo.label} ยังไม่มีข้อมูลรายจ่ายที่บันทึกไว้`;
					...
					let responseText = `${timeInfo.label} จ่ายหมวด${extremeCat}${word} เป็นเงิน ${extremeAmount.toLocaleString()} บาท`;
					if (sortedCats.length > 1) {
						let nextWord = isMin ? "เยอะกว่านั้นมานิดนึง" : "รองลงมา";
						responseText += ` ${nextWord}คือหมวด${sortedCats[1]} ${categorySums[sortedCats[1]].toLocaleString()} บาท`;
					}
					return responseText;
				}
				...
				if (intent === 'budget_left' && specificCategory) {
					...
					if (budgetAmount > 0) {
						...
						if (left < 0) {
							return `สำหรับค่า${specificCategory} ${timeInfo.label} ตั้งงบไว้ ${budgetAmount.toLocaleString()} ใช้ไปแล้ว ${spent.toLocaleString()} เกินงบไป ${Math.abs(left).toLocaleString()} บาทแล้ว`;
						} else {
							return `${timeInfo.label} งบค่า${specificCategory} ใช้ไปแล้ว ${spent.toLocaleString()} คงเหลืองบอีก ${left.toLocaleString()} บาท`;
						}
					} else {
						if (spent === 0) return `${timeInfo.label} คุณยังไม่มีรายจ่ายในหมวด${specificCategory}`;
						return `สำหรับค่า${specificCategory} ${timeInfo.label} คุณใช้ไปแล้ว ${spent.toLocaleString()} บาท`;
					}
				}
			...
			const periodThaiMap = {
				...
				'อาทิตย์นี้': 'this_week',
				'สัปดาห์ที่แล้ว': 'last_week',
				'อาทิตย์ที่แล้ว': 'last_week',
				'สัปดาห์ก่อน': 'last_week',
				'เดือนนี้': 'this_month',
			...
			// ฟังก์ชันตั้งค่าวันที่ตามช่วงเวลา (รองรับ Local Date)
			function applyCustomPeriod(periodThai) {
				const today = new Date();
				const startInput = document.getElementById('adv-filter-start');
				const endInput = document.getElementById('adv-filter-end');
				...
				// แปลง periodThai เป็นรหัสภายใน
				const periodCode = periodThaiMap[periodThai];
				...
				switch (periodCode) {
					...
					case 'last_week':
						...
						lastWeekMonday.setDate(today.getDate() - (today.getDay() === 0 ? 6 : today.getDay() - 1) - 7);
						const lastWeekSunday = new Date(lastWeekMonday);
						lastWeekSunday.setDate(lastWeekMonday.getDate() + 6);
						startDate = formatLocalDate(lastWeekMonday);
						endDate = formatLocalDate(lastWeekSunday);
						break;
					case 'this_month':
					...
					case 'this_year':
						...
						break;
					case 'last_year':
						startDate = formatLocalDate(new Date(today.getFullYear() - 1, 0, 1));
						endDate = formatLocalDate(new Date(today.getFullYear() - 1, 11, 31));
						break;
					case 'all':
						startDate = '';
						endDate = '';
			...
			// ฟังก์ชันดึงรายการที่กรองอยู่ ณ ปัจจุบัน
			function getCurrentFilteredTransactions() {
				...
				const filterType = document.getElementById('adv-filter-type').value;
			...
			function handlePeriodFilterCommand(text, lowerText) {
				...
				let type;
				if (typeThai === 'รายรับ') type = 'income';
				else if (typeThai === 'รายจ่าย') type = 'expense';
				else if (typeThai === 'โอนย้าย') type = 'transfer';
				else return false;

				// ========== ตรวจจับเดือน (รวมหลายเดือน) และปี ==========
				// แยกข้อความด้วย "และ", "กับ", หรือ "ถึง"
				const monthParts = rest.split(/\s*(?:และ|กับ|ถึง)\s*/).filter(p => p.length > 0);
				let months = [];
				let year = parseYearFromText(rest); // อาจเป็น null
				const monthKeysSorted = Object.keys(thaiMonthMap).sort((a, b) => b.length - a.length);

				for (let part of monthParts) {
					let cleanPart = part.replace(/เดือน/g, '').trim();
					if (cleanPart === '') continue;
					for (const key of monthKeysSorted) {
						if (cleanPart.includes(key)) {
							months.push(thaiMonthMap[key]);
							break;
						}
					}
				}
				...
				if (months.length > 0) {
					...
					const endDate = new Date(year, maxMonth, 0); // วันสุดท้ายของ maxMonth
					...
					// สร้างข้อความเดือนสำหรับพูด โดยเลือกใช้ "ถึง" หรือ "และ" ตามที่ผู้ใช้พูด
					const hasRange = rest.includes('ถึง');
					const connector = hasRange ? ' ถึง ' : ' และ ';
					const monthNames = months.map(m => Object.keys(thaiMonthMap).find(k => thaiMonthMap[k] === m));
					let periodText;
					if (months.length === 1) {
						periodText = `เดือน${monthNames[0]} พ.ศ. ${year + 543}`;
					} else {
						periodText = `เดือน${monthNames.join(connector)} พ.ศ. ${year + 543}`;
					}

					setTimeout(() => {
						const filtered = getCurrentFilteredTransactions();
						const total = filtered.reduce((sum, tx) => sum + tx.amount, 0);
						speak(`ในช่วง${periodText} มี${typeThai}ทั้งหมด ${formatCurrency(total)}`);
					}, 600);
					return true;
				}

				// กรณีมีปีอย่างเดียว (ไม่มีเดือน)
				if (year !== null) {
					...
					const periodText = `ปี พ.ศ. ${yearThai}`;
				...
				if (!periodThaiMap.hasOwnProperty(rest)) return false;
				...
				setTimeout(() => {
					...
					if (type === 'transfer') {
						...
						const totalEl = type === 'income' ? document.getElementById('dyn-sum-income') : document.getElementById('dyn-sum-expense');
						if (totalEl) {
					...
					const typeName = type === 'income' ? 'รายรับ' : (type === 'expense' ? 'รายจ่าย' : 'โอนย้าย');
					let periodText = rest;
					if (rest === 'ทั้งหมด') periodText = 'ทุกช่วงเวลา';
					speak(`ในช่วง${periodText} มี${typeName}ทั้งหมด ${totalFormatted}`);
				}, 600);
			...
			// ประกาศตัวแปรเก็บ instance ไว้ด้านบน เพื่อให้ลบกราฟเก่าทิ้งได้ตอนเปิดหน้าต่างซ้ำ (ป้องกันบั๊กกราฟซ้อน)
			let manualCharts = {};

			function renderManualMockCharts() {
				// ฟังก์ชันช่วยเคลียร์กราฟเก่า
				const resetChart = (id) => {
					if (manualCharts[id]) {
						manualCharts[id].destroy();
						delete manualCharts[id];
					}
				};

				// สีและฟอนต์ (เหมือนกับที่ใช้ในแอป)
				const textColor = state.isDarkMode ? '#e5e7eb' : '#4b5563';
				const gridColor = state.isDarkMode ? '#374151' : '#e5e7eb';

				// 1. กราฟสรุปภาพรวมรายรับ-รายจ่าย (Pie)
				const ctxOverview = document.getElementById('manualOverviewChart');
				if (ctxOverview) {
					resetChart('overview');
					manualCharts['overview'] = new Chart(ctxOverview, {
						type: 'pie',
						data: {
						...
						options: {
							...
							plugins: {
								...
								tooltip: {
									callbacks: {
										label: (ctx) => ` ${ctx.label}: ${new Intl.NumberFormat('th-TH', { style: 'currency', currency: 'THB' }).format(ctx.raw)}`
									}
								}
							}
						}
				...
				// 2. กราฟ Top 10 รายจ่าย (Pie)
				const ctxTop10 = document.getElementById('manualTop10Chart');
				if (ctxTop10) {
					resetChart('top10');
					// จำลอง Top 6 รายการ (รวม "อื่นๆ")
					manualCharts['top10'] = new Chart(ctxTop10, {
						type: 'pie',
						data: {
							labels: ['อาหาร', 'ช้อปปิ้ง', 'เดินทาง', 'บ้าน', 'สาธารณูปโภค', 'อื่นๆ'],
							datasets: [{
								data: [12000, 8000, 5000, 4500, 2500, 3000],
								backgroundColor: ['#e11d48', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#8b5cf6'],
								borderWidth: 1
							}]
						},
						options: {
				...
				// 3. กราฟหมวดหมู่ (Doughnut) – สำหรับหน้ารายการ
				const ctxCategory = document.getElementById('manualAccountsChart');
				if (ctxCategory) {
					resetChart('category');
					manualCharts['category'] = new Chart(ctxCategory, {
						type: 'doughnut',
						data: {
							labels: ['อาหาร', 'ช้อปปิ้ง', 'เดินทาง', 'บ้าน', 'อื่นๆ'],
							datasets: [{
								data: [12000, 8000, 5000, 4500, 2500],
				...
				// 4. กราฟแนวโน้ม 7 วัน (Bar) – สำหรับหน้ารายการ
				const ctxTrend = document.getElementById('manualTrendChart');
				if (ctxTrend) {
					resetChart('trend');
					manualCharts['trend'] = new Chart(ctxTrend, {
						type: 'bar',
						data: {
							labels: ['จ', 'อ', 'พ', 'พฤ', 'ศ', 'ส', 'อา'],
							datasets: [{
								label: 'ยอดใช้จ่าย',
								data: [4500, 6200, 3800, 5100, 4700, 6900, 5300],
								backgroundColor: '#8b5cf6', // สีม่วงตามแอป
								borderRadius: 4,
								barPercentage: 0.7
							}]
						},
			...
			// ==========================================================
			// [NEW FUNCTION] ระบบจัดการการติดตั้งแอป (อัปเดตแจ้งเตือนติดตั้งแล้ว & iOS ขั้นตอนละเอียด)
			// ==========================================================
			let deferredInstallPrompt = null;

			// 1. ดักจับ Event เมื่ออุปกรณ์พร้อมให้ติดตั้ง
			window.addEventListener('beforeinstallprompt', (e) => {
				e.preventDefault();
				deferredInstallPrompt = e;
			});

			// 2. ดักจับ Event เมื่อ "ผู้ใช้ติดตั้งแอปเสร็จสมบูรณ์" (เปลี่ยนสีปุ่มทันที)
			window.addEventListener('appinstalled', () => {
				console.log('PWA: Installed successfully');
				deferredInstallPrompt = null;
				updateButtonToInstalled();
			});

			// 3. ฟังก์ชันสำหรับเปลี่ยนหน้าตาปุ่มเป็น "สีเขียว (ติดตั้งแล้ว)"
			function updateButtonToInstalled(isInsideApp = false) {
				const btnInstall = document.getElementById('btn-pwa-install');
				const txtStatus = document.getElementById('txt-pwa-status');
				
				if (btnInstall) {
					btnInstall.innerHTML = '<i class="fa-solid fa-circle-check"></i> <span>ติดตั้งเรียบร้อยแล้ว</span>';
					btnInstall.classList.remove('from-purple-600', 'to-indigo-600', 'hover:from-purple-700', 'hover:to-indigo-700');
					btnInstall.classList.add('bg-green-500', 'hover:bg-green-600', 'text-white', 'is-installed-btn');
				}
				
				if (txtStatus) {
					txtStatus.textContent = isInsideApp ? "คุณกำลังใช้งานผ่านแอปพลิเคชัน" : "แอปถูกติดตั้งลงบนเครื่องแล้ว";
				}
			}

			// 4. ตรวจสอบสถานะตอนเริ่มโหลดหน้าเว็บ
			...
			// 5. จัดการเมื่อกดปุ่ม (Event Delegation)
			document.addEventListener('click', async (e) => {
				const btn = e.target.closest('#btn-pwa-install') || e.target.closest('#btn-welcome-install');
				if (!btn) return;

				const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone;

				// --- กรณีที่ 1: เปิดใช้งานผ่านแอปอยู่แล้ว ---
				if (isStandalone) {
					Swal.fire({
						title: 'คุณอยู่ในแอปแล้ว',
						text: 'ขณะนี้คุณกำลังใช้งานหน้าต่างแอปพลิเคชันอยู่แล้วครับ 🎉',
						icon: 'success',
						confirmButtonColor: '#10b981'
					});
					return;
				...
				// --- กรณีที่ 2: ปุ่มเป็นสีเขียวไปแล้ว แต่ยังอยู่บนเว็บ ---
				if (btn.classList.contains('is-installed-btn')) {
					Swal.fire({
						title: 'ติดตั้งแอปแล้ว!',
						html: 'แอปถูกติดตั้งไว้ที่ <b>"หน้าจอโฮม"</b> ของคุณเรียบร้อยแล้ว<br><br><span class="text-sm text-gray-500">กรุณาปิดหน้าเว็บนี้ และไปกดเปิดแอปจากไอคอนบนหน้าจอโทรศัพท์ของคุณครับ</span>',
						icon: 'info',
				...
				// --- กรณีที่ 3: ระบบรองรับการเด้งติดตั้งอัตโนมัติ (ยังไม่เคยติดตั้ง) ---
				...
				// --- กรณีที่ 4: เบราว์เซอร์ไม่เด้งให้ติดตั้ง (iOS หรือ ติดตั้งไปแล้วใน Android) ---
				const isIOS = /iphone|ipad|ipod/.test(navigator.userAgent.toLowerCase());
				if (isIOS) {
					Swal.fire({
						title: 'การติดตั้งบน iOS',
						html: `
							<div class="text-left text-sm space-y-3 px-2 mb-2">
								<p class="text-gray-600 mb-2">หากคุณยังไม่ได้ติดตั้งแอป กรุณาทำตามขั้นตอนดังนี้:</p>
								<p class="flex items-start gap-2">
									<span class="bg-gray-200 text-gray-700 rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">1</span>
									<span>กดปุ่ม <strong>แชร์</strong> <i class="fa-solid fa-arrow-up-from-bracket text-blue-500"></i> ที่แถบด้านล่างของ Safari</span>
								</p>
								<p class="flex items-start gap-2">
									<span class="bg-gray-200 text-gray-700 rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">2</span>
									<span>เลื่อนหาเมนู <strong>"เพิ่มไปยังหน้าจอโฮม"</strong> <i class="fa-regular fa-square-plus text-gray-500"></i> (Add to Home Screen)</span>
								</p>
								<p class="flex items-start gap-2">
									<span class="bg-gray-200 text-gray-700 rounded-full w-5 h-5 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">3</span>
									<span>กดคำว่า <strong>"เพิ่ม"</strong> (Add) ที่มุมขวาบน</span>
								</p>
							</div>
							<div class="mt-4 bg-purple-50 p-3 rounded-lg border border-purple-100 text-xs text-purple-700 text-left">
								<i class="fa-solid fa-circle-info mr-1"></i> <b>หากติดตั้งไปแล้ว:</b><br>
								สามารถปิดเบราว์เซอร์และไปเปิดแอปจากหน้าจอโฮมได้เลยครับ
							</div>
						`,
						icon: 'info',
					...
					// กรณีไม่ใช่ iOS และไม่มี Prompt (แปลว่าติดตั้งไปแล้วแน่นอนบน Android/PC)
					Swal.fire({
						title: 'ติดตั้งแอปแล้ว!',
						html: `
							<div class="text-left text-sm text-gray-700 space-y-2">
								<p>ระบบตรวจพบว่า <b>คุณได้ทำการติดตั้งแอปพลิเคชันนี้ไปแล้ว</b></p>
								<p>กรุณาปิดหน้าเว็บเบราว์เซอร์นี้ และไปเปิดแอปพลิเคชันจาก <b>"หน้าจอโฮม"</b> บนอุปกรณ์ของคุณได้เลยครับ</p>
								<hr class="my-3 border-gray-200">
								<p class="text-[11px] text-gray-400">
									*หากยังไม่พบไอคอนแอป กรุณากดที่เมนูของเบราว์เซอร์ (ไอคอน 3 จุด) แล้วเลือก "ติดตั้งแอป" หรือ "เพิ่มไปยังหน้าจอโฮม"
								</p>
							</div>
						`,
						icon: 'success', // เปลี่ยนเป็น Success เพราะถือว่าเขาติดตั้งเสร็จแล้ว
						confirmButtonText: 'ตกลง',
						confirmButtonColor: '#10b981' // ปุ่มสีเขียว
					});
				}
			});
			
			// ==========================================================
			// [NEW FUNCTION] ระบบหน้าจอต้อนรับ (Welcome Screen) - ฉบับจำสถานะ
			// ==========================================================
			...
			window.addEventListener('load', () => {
				...
				// --- ฟังก์ชันช่วยเหลือ: เปลี่ยนปุ่มให้เป็นสีเขียว (ติดตั้งแล้ว/ทำแล้ว) ---
				function markWelcomeButtonAsDone(btnId, title, subtitle) {
					const btn = document.getElementById(btnId);
					if (!btn) return;
					btn.innerHTML = `
						<div class="flex items-center gap-4">
							<div class="w-10 h-10 rounded-full bg-green-500 text-white flex items-center justify-center text-lg shadow-sm">
								<i class="fa-solid fa-check"></i>
							</div>
							<div class="text-left">
								<span class="block text-sm font-bold text-green-700 dark:text-green-400">${title}</span>
								<span class="block text-[11px] text-green-600/70 font-normal mt-0.5">${subtitle}</span>
							</div>
						</div>
					`;
					btn.classList.add('pointer-events-none', 'bg-green-50', 'border-green-200', 'dark:bg-green-900/20', 'dark:border-green-800');
				}

				// ตรวจสอบว่าเคยเข้าใช้งานหน้าต่างนี้แล้วหรือยัง?
				const hasSeenWelcome = localStorage.getItem('hasSeenWelcomeModal_FMPro');
				
				// ถ้ายังไม่เคยดู ให้ตรวจสอบสถานะปุ่มและแสดงหน้าต่าง
				if (!hasSeenWelcome) {
					setTimeout(() => {
						welcomeModal.classList.remove('hidden');
						
						// เช็คสถานะ 1: ถ้าผู้ใช้เปิดผ่านแอปที่ติดตั้งแล้ว ให้ปุ่มติดตั้งเป็นสีเขียวเลย
						const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone || document.referrer.includes('android-app://');
						if (isStandalone) {
							markWelcomeButtonAsDone('btn-welcome-install', 'ติดตั้งแอปลงเครื่องแล้ว', 'กำลังใช้งานผ่านแอปพลิเคชัน');
						}
						
						// เช็คสถานะ 2: ถ้าผู้ใช้เคยกดตั้งรหัสผ่านผ่านหน้านี้ไปแล้ว ให้ปุ่มเป็นสีเขียว
						if (localStorage.getItem('welcome_pin_done') === 'true') {
							markWelcomeButtonAsDone('btn-welcome-password', 'ตั้งรหัสผ่านเรียบร้อยแล้ว', 'ระบบถูกปกป้องแล้ว');
						}
			...
			// 2. ดักจับการกดปุ่มในหน้า Welcome
			document.addEventListener('click', (e) => {
				
				// --- 2.1 ถ้ายิงโดนปุ่ม "ข้าม / ปิด" ---
				if (e.target.closest('#btn-welcome-close')) {
					const welcomeModal = document.getElementById('welcome-modal');
					if (welcomeModal) {
						welcomeModal.classList.add('hidden'); // ปิดหน้าต่าง
					}
					// บันทึกว่าดูหน้าต้อนรับจบแล้ว
					localStorage.setItem('hasSeenWelcomeModal_FMPro', 'true');
					return;
				}

				// --- 2.2 ถ้ายิงโดนปุ่ม "ตั้งรหัสผ่าน" ---
				const btnPassword = e.target.closest('#btn-welcome-password');
				if (btnPassword) {
					const originalPinBtn = document.getElementById('btn-manage-password');
					
					if (originalPinBtn) {
						// เทคนิคพิเศษ: ดักจับการทำงานของหน้าต่างแจ้งเตือน (SweetAlert) ชั่วคราว
						const originalSwalFire = Swal.fire;
						let isSwalCalled = false;

						Swal.fire = function(...args) {
							isSwalCalled = true;
							Swal.fire = originalSwalFire; // คืนค่าระบบกลับเป็นปกติทันทีเพื่อไม่ให้กระทบส่วนอื่นของแอป

							// รอจนกว่าผู้ใช้จะกดอะไรสักอย่างในหน้าต่างตั้งรหัส
							return originalSwalFire.apply(this, args).then((result) => {
								
								// สำคัญตรงนี้: ถ้าผู้ใช้กด "ตกลง/บันทึก" (ไม่กด Cancel หรือกดปิดหน้าต่างทิ้ง)
								if (result.isConfirmed) {
									
									// เปลี่ยนปุ่มหน้า Welcome เป็นสีเขียว
									btnPassword.innerHTML = `
										<div class="flex items-center gap-4">
											...
											<div class="text-left">
												<span class="block text-sm font-bold text-green-700 dark:text-green-400">ตั้งรหัสผ่านเรียบร้อยแล้ว</span>
												<span class="block text-[11px] text-green-600/70 font-normal mt-0.5">ระบบถูกปกป้องแล้ว</span>
											</div>
										</div>
									`;
									// ล็อคปุ่มไม่ให้กดซ้ำ
									btnPassword.classList.add('pointer-events-none', 'bg-green-50', 'border-green-200', 'dark:bg-green-900/20', 'dark:border-green-800');
									
									// บันทึกความจำว่าผ่านขั้นตอนนี้แล้ว
									localStorage.setItem('welcome_pin_done', 'true');
									localStorage.setItem('hasSeenWelcomeModal_FMPro', 'true');
								}
								
								return result; // ส่งค่ากลับไปให้ระบบตั้งรหัสผ่านเดิมทำงานต่อให้จบ
							});
						};

						// สั่งจำลองการคลิกไปที่ปุ่มจัดการรหัสผ่านเดิมของคุณ
						originalPinBtn.click();

						// ระบบป้องกัน: ถ้าปุ่มเดิมไม่ยอมเรียกหน้าต่างขึ้นมาภายใน 1 วินาที ให้คืนค่าระบบกลับ
						setTimeout(() => {
							if (!isSwalCalled) Swal.fire = originalSwalFire;
						}, 1000);

					} else {
						console.error("ไม่พบปุ่ม btn-manage-password ในระบบ");
					}
					
					return;
				}

				// --- 2.3 ถ้ายิงโดนปุ่ม "ติดตั้งแอป" (ส่วนทำงานติดตั้งอยู่ในโค้ด PWA แล้ว) ---
				const btnInstall = e.target.closest('#btn-welcome-install');
				if (btnInstall) {
					// บันทึกว่าใช้งานหน้าต้อนรับแล้ว (รีเฟรชครั้งหน้าจะได้ไม่เด้ง)
					localStorage.setItem('hasSeenWelcomeModal_FMPro', 'true');
				}
			});
			
			// =========================================================================
			// ฟังก์ชันเสริม: ระบบจัดการการตั้งค่าแจ้งเตือน (รายการล่วงหน้า, รายการประจำ, งบประมาณ)
			// =========================================================================
			document.addEventListener('DOMContentLoaded', () => {
